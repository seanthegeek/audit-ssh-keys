...
- **Keep the stdlib-only, single-module shape.** No third-party runtime dependencies; the tool is copied to hosts that may have nothing but `python3` and `openssh-client`. Keep `src/audit_ssh_keys/audit.py` as the one module unless it becomes genuinely unwieldy. Every release also ships `audit.py` on its own, renamed to `audit-ssh-keys.py`, so `audit.py` must never import anything from the rest of the package; `__version__` lives in `audit.py`, with `__init__.py` re-exporting it, so a standalone copy still reports a real version. `test_audit_py_runs_standalone_without_the_package` and the release workflow's gate both run the copy under `python -S`, which catches an unguarded import; a guarded one would slip through, so do not add one.
