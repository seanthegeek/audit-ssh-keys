...
- **Every ssh-keygen call gets `stdin=subprocess.DEVNULL` (or an explicit `input=`), and the one call that loads a private half, `ssh-keygen -y`, always gets `-P ""`.** Nothing in this tool may ever block on a passphrase prompt. Closing stdin is not enough on its own: `ssh-keygen` asks for passphrases through `/dev/tty`, or through the `SSH_ASKPASS` program when a desktop session is set, so a bare `ssh-keygen -y` on a passphrase-protected (or corrupt) key pops a GUI dialog on the operator's screen. Handing over an empty passphrase with `-P ""` makes it fail immediately instead. This applies to experiments run while developing, too — a stray `ssh-keygen -y` in a test harness has already interrupted a real desktop session.
...
- **Back up any database before any schema or migration change.** Before running any schema-changing SQL (ALTER TABLE, CREATE/DROP, hand-rolled column rewrites, anything that mutates table shape) against a database, back it up.
