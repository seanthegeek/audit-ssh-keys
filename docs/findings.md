...
| `not modified in N days` | LOW | Only reported when `--authorized-keys-unchanged-for N` is given, and only for a file holding at least one key `sshd` would actually match — a file of comments, a file holding nothing but certificate lines (which authorise nobody), and a file the tool could not read say nothing about whose access is old. Every key in the file has been there at least that long. It says how old the file's keys are, not whether anybody has looked at them: a review that changes nothing leaves the modification time where it was | Check each key in the file against the people and systems that should still have access, and remove the ones that should not |
