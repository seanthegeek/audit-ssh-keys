...
| `modified within the last N days` | MEDIUM | Only appears with `--host-keys-changed-within`. An opt-in tripwire: the key's modification time falls inside the window you chose, so either this was a planned rotation or it is exactly the unexpected replacement you were looking for. Per-file mtime, and a copy tool or `ssh-keygen -A` resets it, so this is a first pass, not proof | Confirm who rotated it and why; compare the fingerprint clients now see against the one recorded before the change |
...
| `not modified in N days` | LOW | Only appears with `--authorized-keys-unchanged-for`. The file's contents have not changed for that long, so every key in it is at least that old and nobody has reviewed the list since. Modification time is per file, not per key, and copy tools (`rsync -a`, `cp -p`, config management) can carry an old time onto a fresh file or a new time onto old content | Review the keys, remove any that are no longer needed, and rotate the rest |
| `modified within the last N days` | MEDIUM | Only appears with `--authorized-keys-changed-within`. An opt-in tripwire: the file's modification time falls inside the window you chose, so this is either a planned edit or the unexpected change you were looking for. Fires on every scanned file regardless of key count, since keys removed or the file emptied are changes too. Per-file mtime, and a copy tool or config management can reset it, so this is a first pass, not proof | Confirm who changed it and why |
