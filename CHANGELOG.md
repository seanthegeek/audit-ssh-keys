...
- `--json` output for fleet rollups; `-v/--verbose` to list every key including clean ones; `--skip-host`, `--skip-authorized`, `--skip-private`, `--min-rsa-bits`, `--authorized-keys-unchanged-for`, `--authorized-keys-changed-within`, `--host-keys-changed-within`, `--debug`
- Test suite and docs
